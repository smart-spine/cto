...
3. Delegate via guarded wrapper and include exact line: `Write Unit Tests & Verify`.
   - `python3 ${OPENCLAW_STATE_DIR:-$HOME/.openclaw}/workspace-factory/scripts/codex_guarded_exec.py --workdir <root_project_workspace> --model gpt-5.3-codex --prompt-file <prompt_file> --retries 3 --timeout 420`
...
9. Report evidence: delegation method, command, exit code, test commands, test exit codes, artifact-gate result, and `openclaw.json` validation results.
