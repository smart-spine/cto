...
def main() -> int:
    ...
    p.add_argument("--menu-command", default="/pain_menu")
    ...
    for f in runtime_files:
        txt = read_text(f)
        has_menu = has_any(txt, [args.menu_command, "menu"])  # defensive fallback
        has_callback = has_any(txt, [args.callback_namespace, "callback_data", "callback"])  # callback routing
        has_button_send = has_any(txt, ["inline_keyboard", "reply_markup", "buttons", "message send", "send_message("])

        if has_menu and has_button_send:
    ...
    if not menu_handler_files:
        result["failures"].append("no runtime file proves menu->button-send path")
    ...
    if not button_send_files:
        result["failures"].append("no runtime file with button payload/send logic found")
    ...
    for f in test_files:
        txt = read_text(f)
        if has_any(txt, [args.menu_command, "menu"]) and has_any(
    ...
    if prompts_md.exists():
        ...
        if not has_any(txt, ["fallback", "button", "menu"]):
